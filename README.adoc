...
You can run your workloads in a Kubernetes cluster with different cloud-based solutions. 
To focus on developing your microservices without worrying about details related to the servers you deploy them to, 
you can enable a cloud-based infrastructure. Using a cloud helps 
...
and distribute your container images. Note, since AKS is not free, a small
...
To create your AKS cluster, use the `az aks create` cluster command. You might need to wait while your
...
a service principal to use with your newly created cluster. However, to run this command, your Azure account must have permission access to create
service principals. 

Optionally, you can create a service principal with the `az ad sp create-for-rbac` command and create a cluster with this new service principal. 
...
`az acr credential show -n guideRegistry` command. The email that is associated with your Docker account replaces `[email-address]`.
...
hostname that you will later substitute into the `[hostname]` credential.
...
When the `EXTERNAL-IP` appears, press `CTRL-C` for kubectl to stop watching the `system` service. 
...
When the `EXTERNAL-IP` appears, press `CTRL-C` for kubectl to stop watching the `inventory` service.
...
It is important to clean up your resources when you are finished with the guide so that you do not incur extra charges for ongoing service.
