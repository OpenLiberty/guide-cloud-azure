...
:page-seo-title: Deploying microservices to Microsoft Azure Kubernetes Service (AKS)
:page-seo-description: A tutorial on how to deploy microservices to Microsoft AKS using Azure Container Registry (ACR) as a private container registry.
...
cost that is associated with deploying this application. See the official
...
It also returns the pod’s name in the HTTP header, making replicas easy to distinguish from each other. 
...
where your services will be deployed, and link the registry to a resource group. Your registry will manage container instances 
...
If resources are created later without specifying a location, these new resources will run in the
...
These different SKU options provide pricing for various levels of capacity and usage. You are using a `Basic` SKU because it is cheaper and 
the services you will deploy have low storage and throughput requirements.
...
You must log into your Azure Container Registry using the Azure CLI, in order to push Docker images to your registry.
...
https://openliberty.io/guides/containerize.html[Using Docker containers to develop microservices^] guide.
...
[role=command]
```
docker tag system:1.0-SNAPSHOT guideregistry.azurecr.io/system:1.0-SNAPSHOT
docker tag inventory:1.0-SNAPSHOT guideregistry.azurecr.io/inventory:1.0-SNAPSHOT
```
...
[role=command]
```
docker push guideregistry.azurecr.io/system:1.0-SNAPSHOT
docker push guideregistry.azurecr.io/inventory:1.0-SNAPSHOT
```
...
To create your AKS cluster, use the `az aks create` cluster command. You may need to wait while your
...
An AKS cluster requires a service principal, which is an identity used to represent a resource in Azure that 
...
a service principal to use with your newly created cluster. However, in order to run this command, your Azure account must have permission access to create
service principals. 

Optionally, you can create a service principal with the `az ad sp create-for-rbac` command and create a cluster with this new service prinicipal. 
The `az ad sp create-for-rbac` command manually creates a service principal and outputs information about it, including an `appId` and `password`.
In the `az aks create` cluster creation command, these two values can be passed into the additional arguments, `--service-principal` and `--client-secret`: 
...
Merge the credentials of your cluster into your current Kubernetes configuration using the `az aks get-credentials` command. The default Kubernetes configuration file that is updated with your cluster credentials is located at `~/.kube/config`. 
...
The service used to expose your deployments has a type of [hotspot=sysLoadBalancer hotspot=invLoadBalancer file=0]`LoadBalancer`. 
...
Note that there are other ways to expose your services such as using a `NodePort` service type. 
...
You will need the external IP addresses associated with the `system` and `inventory` services to try out your microservices.
...
Once the `EXTERNAL-IP` appears, press `CTRL-C` for kubectl to stop watching the `system` service. 
...
Once the `EXTERNAL-IP` appears, press `CTRL-C` for kubectl to stop watching the `inventory` service.
...
Run the Maven `verify` goal to test your microservices by replacing the `[system-EXTERNAL-IP]` and `[inventory-EXTERNAL-IP]`
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.673 sec - in it.io.openliberty.guides.system.SystemEndpointTest
