...
:page-related-guides: ['kubernetes-intro', 'kubernetes-microprofile-config', 'docker']
...
Your login credentials will be needed later, so keep your given password in mind. View the password for your
...
Optionally, service principals can also be created manually with the `az ad sp create-for-rbac` command. This command outputs information about the
...
Replace `[password]` with the registry password that you viewed in the last section by running the 
