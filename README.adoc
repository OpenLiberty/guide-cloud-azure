...
These different SKU options provide pricing for various levels of capacity and usage. You are using a `Basic` SKU because
it has a cheaper cost and your application involves a lower usage of storage and image throughput.
...
Optionally, a cluster can be created with existing service principals. The `az ad sp create-for-rbac` command manually creates a service principal 
and outputs information about it, including an `appId` and `password`.
